...
public final class WFConnectorHitArea {
	...
	/** Maximum step length along either axis of a diagonal segment. */
	...
	/**
	 * Add overlapping stepped hit rectangles for a diagonal connector segment. Diagonal
	...
	private static void addDiagonal(List<Rectangle> areas, Point from, Point to, List<Rectangle> cardRects) {
		...
		int steps = (distance + DIAGONAL_STEP - 1) / DIAGONAL_STEP;
		if (steps < 1)
			steps = 1;
		for (int i = 0; i < steps; i++) {
			int x1 = from.x + dx * i / steps;
			int y1 = from.y + dy * i / steps;
			int x2 = from.x + dx * (i + 1) / steps;
			int y2 = from.y + dy * (i + 1) / steps;
			Rectangle area = new Rectangle(Math.min(x1, x2) - RADIUS, Math.min(y1, y2) - RADIUS,
					Math.abs(x2 - x1) + 2 * RADIUS, Math.abs(y2 - y1) + 2 * RADIUS);
			List<Rectangle> current = new ArrayList<Rectangle>();
			current.add(area);
			for (Rectangle r : cardRects) {
				List<Rectangle> next = new ArrayList<Rectangle>();
