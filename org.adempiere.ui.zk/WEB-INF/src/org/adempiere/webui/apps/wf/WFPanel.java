...
public class WFPanel extends Borderlayout implements EventListener<Event>, IHelpContext
{
	...
	public void load (int AD_Workflow_ID)
	{
		...
		double renderScale = WFRenderUtil.getRenderScale(dimension.width, dimension.height);
		int imageWidth = (int) Math.floor(dimension.width * renderScale);
		int imageHeight = (int) Math.floor(dimension.height * renderScale);
