...
public class WFEditor extends ADForm {
	...
	private void load(int workflowId, boolean reread) {
		...
		double renderScale = WFRenderUtil.getRenderScale(imageWidth, imageHeight);
		BufferedImage bi = new BufferedImage ((int) Math.floor(imageWidth * renderScale),
				(int) Math.floor(imageHeight * renderScale), BufferedImage.TYPE_INT_ARGB);
		...
		paintGraphics.scale(renderScale, renderScale);
		...
		try {
			...
			// (up to WFRenderUtil.RENDER_SCALE times larger than the logical size
			// to stay sharp on HiDPI). Tiling it into per-cell <img> parts would
...
String style = "position:absolute;left:" + (WFGraphLayout.MARGIN_X + c * WFGraphLayout.COLUMN_WIDTH) + "px;"
					...
					if (widget != null)
					{
						...
						try {
							...
							BufferedImage dragCard = createDragCardImage(node);
							if (dragCard != null) {
								org.zkoss.zul.Image cardImg = new org.zkoss.zul.Image();
								cardImg.setContent(dragCard);
	...
	/**
	 * Render one clean node card for use as its drag image. Rendering a small
	 * independent scene avoids retaining a second full workflow raster.
	 * @param node workflow node represented by the card
	 * @return supersampled card image
	 */
	private BufferedImage createDragCardImage(MWFNode node) {
		int width = WFNodeWidget.NODE_WIDTH + 2 * CARD_DRAG_MARGIN;
		int height = WFNodeWidget.NODE_HEIGHT + 2 * CARD_DRAG_MARGIN;
		BufferedImage image = new BufferedImage(
				(int) Math.round(width * WFRenderUtil.RENDER_SCALE),
				(int) Math.round(height * WFRenderUtil.RENDER_SCALE), BufferedImage.TYPE_INT_ARGB);
		...
		try {
			...
			Scene scene = new Scene() {
				...
				public java.awt.Font getDefaultFont() {
					return nodeContainer.getGraphScene().getDefaultFont();
				}
			};
			scene.setMaximumBounds(new Rectangle(0, 0, width, height));
			WFNodeWidget dragWidget = new WFNodeWidget(scene, node);
			dragWidget.setShadowEnabled(false);
			dragWidget.setPreferredLocation(new Point(CARD_DRAG_MARGIN, CARD_DRAG_MARGIN));
			scene.addChild(dragWidget);
			scene.validate(graphics);

			Graphics2D paintGraphics = (Graphics2D) graphics.create();
			try {
				paintGraphics.scale(WFRenderUtil.RENDER_SCALE, WFRenderUtil.RENDER_SCALE);
				scene.paint(paintGraphics);
			} finally {
				paintGraphics.dispose();
			}
		return image;
